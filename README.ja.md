...
ほとんどのコマンドは `--json` に対応しており、プログラムが読み取れる形式で結果を出力できます — ただし `write`/`edit`/`patch`/`archive`/`ingest` は例外で、成功・失敗をプレーンテキストでのみ報告します。「読む」系のコマンドはたいてい、デフォルトでは短い要約だけを表示します(全部見たいときは `--full`/`--no-brief` を付けてください)。
