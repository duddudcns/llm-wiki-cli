...
这样装还会附带三个安全保障机制：一个负责自动让命令行工具本身保持最新,一个负责防止 AI 绕开 wiki、直接改文件,还有一个会提醒 AI 在开始新工作前先去搜一搜 wiki——具体是怎么做的、以及不想要的话怎么关掉,可以看 [hooks.md](hooks.md)。如果你更想自己安装命令行工具、自己手动管理更新,可以跳过这一步,改用下面的方法——两者不会冲突,你可以两个都装。
