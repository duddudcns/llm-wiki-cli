...
플러그인이 고정된 GitHub 릴리스에서 MCP 서버를 자동 실행하므로 `llmw` CLI를 별도로 설치할 필요는 없습니다. 직접 터미널에서도 사용하려는 경우에만 아래 CLI 설치를 추가로 진행하세요.

이렇게 설치하면 안전장치 세 가지도 함께 들어옵니다. 하나는 커맨드라인
도구 자체를 자동으로 최신 상태로 유지해 주고, 다른 하나는 AI가 위키를
건너뛰고 파일을 직접 고치지 못하게 막아주고, 나머지 하나는 새 작업을
시작하기 전에 위키를 먼저 검색하도록 AI에게 알려줍니다. 정확히 어떤
기능인지, 원하지 않으면 어떻게 끄는지는 [hooks.md](hooks.md)를 보세요. 커맨드라인
