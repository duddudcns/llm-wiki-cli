...
これを実行すると、3つの安全装置も一緒にインストールされます。1つはコマンドラインツール自体を自動的に最新の状態に保つもの、1つはAIがwikiを飛ばして直接ファイルを編集してしまうのを防ぐもの、そしてもう1つは新しい作業を始める前にwikiを確認するようAIに促すものです。それぞれが具体的に何をするか、そしてオフにしたい場合の方法は [hooks.md](hooks.md) を見てください。もしコマンドラインツールは自分で入れて、アップデートも自分で管理したいなら、これは飛ばして下の方法のどれかを使ってください。両方インストールしても問題ありません、干渉し合うことはありません。

## コマンドラインツール単体でインストール(プラグインなし)

Claude Codeの外で `llmw` を使いたい場合(スクリプトの中で使う、自動処理の一部にする、他のエディターやツールと組み合わせるなど)は、こちらを選んでください。

`llmw` を動かすには **Python 3.11以降** が必要です。まだ一般公開のパッケージ置き場には登録されていないので、代わりにこのGitHubリポジトリから直接インストールします。**現時点でこのリポジトリは非公開です** — インストール(以下のどの方法でも)には、自分のGitHubアカウントで `git` にログインできる状態が必要です(たとえば `gh auth login` で既にログインしている、またはGitHubアカウントにSSHキーを登録済みなど)。この準備がないと、中途半端に壊れたものが入るのではなく、はっきりしたエラーでインストールが失敗します。
