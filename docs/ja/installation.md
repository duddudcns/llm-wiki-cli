...
これを実行すると、2つの安全装置も一緒にインストールされます。1つはコマンドラインツール自体を自動的に最新の状態に保つもの、もう1つはAIがwikiを飛ばして直接ファイルを編集してしまうのを防ぐものです。それぞれが具体的に何をするか、そしてオフにしたい場合の方法は [hooks.md](hooks.md) を見てください。もしコマンドラインツールは自分で入れて、アップデートも自分で管理したいなら、これは飛ばして下の方法のどれかを使ってください。両方インストールしても問題ありません、干渉し合うことはありません。
