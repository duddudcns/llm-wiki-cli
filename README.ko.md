...
- **작업하다가 나온 선호도도 알아서 챙깁니다** — 일하다가 지나가는 말로 코딩 컨벤션이나 정정 사항을 언급하면, "기억해 둬" 나 "위키 업데이트해"라고 따로 말하지 않아도 AI가 위키나 자체 규칙 파일에 기록해 둡니다. 매번 그렇게 말해야만 쓸모 있는 도구라면, 좋은 도구라고 할 수 없죠.
...
.llmw/                        # 뒤에서 돌아가는 검색 인덱스(llmw rebuild로 만듦, 언제든 다시 만들 수 있음)
.claude/skills/llm-wiki/      # Claude Code에게 이 도구 쓰는 법을 알려줌
.claude/rules/llm-wiki.md     # 작업 전 검색·후 업데이트를 자동으로 유도하는 규칙
.codex/rules/llm-wiki.md      # 같은 안내를 Codex용으로(어느 플러그인을 쓰든 매번 생성됨)
...
대부분의 명령어는 프로그램이 읽기 좋은 형식으로 결과를 받고 싶다면 `--json`을 지원합니다 — 단 `write`/`edit`/`patch`/`archive`/`ingest`는 예외로, 성공/실패를 일반 텍스트로만 알려줍니다. 대부분의 "읽기" 명령어는 기본적으로 짧은 요약만 보여줍니다(전체 내용을 보려면 `--full`이나 `--no-brief`를 붙이세요).
